...
import { getUser } from '@/modules/auth/auth-user'
import { getUserFromServer } from '@/modules/auth/auth-server'

vi.mock('@/modules/auth/auth-server', () => ({ getUserFromServer: vi.fn() }))
...
beforeEach(() => {
  vi.mocked(getUserFromServer).mockReset()
  getClaims.mockReset()
...
describe('getUser', () => {
  ...
  it('falls back to the server function during SSR, which has no browser crypto', async () => {
    vi.mocked(getUserFromServer).mockResolvedValue({
      id: 'user-id',
      email: 'camille@example.com',
      fullName: null,
      jobTitle: null,
    ...
    expect(getUserFromServer).toHaveBeenCalledTimes(1)
