...
`getUserFromServer` used to call `supabase.auth.getUser()` and pay a network round-trip on every
navigation — the
...
died while `getUserFromServer` and `provisionUser` were still running, which is what produced the blank
